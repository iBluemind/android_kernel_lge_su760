...
int broadcast_drv_if_power_on(void)
{
    ...
    if(MscBuffer == NULL)
    {
        MscBuffer = kmalloc(TDMB_MSC_BUFFER_SIZE * TDMB_MSC_BUF_CHUNK_NUM, GFP_KERNEL);
        TcbdInitQueue(&MscQueue, TDMB_MSC_BUF_CHUNK_NUM,  MscBuffer, TDMB_MSC_BUFFER_SIZE * TDMB_MSC_BUF_CHUNK_NUM);
    }
...
int broadcast_drv_if_get_dmb_data(char** buffer_ptr, unsigned int* buffer_size, unsigned int user_buffer_size)
{
    ...
    {
        printk("user buffer is not enough user:%d, buffer_size:%d\n", user_buffer_size, *buffer_size);
        return ERROR;
    }
...
int broadcast_drv_if_user_stop()
{
    TcbdResetQueue(&MscQueue);
    tunerbb_drv_tcc3170_set_userstop();

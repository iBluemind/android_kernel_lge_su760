...
#define TCBD_FIC_QUEUE_SIZE 10
static I08U FicBuffer[TCBD_FIC_BUFFER_SIZE];
...
#if defined(STREAM_SPI_UPLOAD)    // if SPI interface
    TcbdInitQueue(&FicQueue, TCBD_FIC_QUEUE_SIZE,  FicBuffer, TCBD_FIC_BUFFER_SIZE);
...
int8 tunerbb_drv_tcc3170_get_ber(struct broadcast_tdmb_sig_info *dmb_bb_info)
{
    ...
    ret = TcbdReadSignalInfo(&TcbdDevice, &SignalInfo);
    ...
    dmb_bb_info->sync_lock = dmb_bb_info->tp_lock;
    dmb_bb_info->tp_err_cnt = SignalInfo.tsErrorCnt;

    TcbdDebug(0, "lock:0x%X, rssi=%d, snr:%d, pcber:%d, vber:%d, tsper:%d\n", SignalInfo.lock,
...
int8 tunerbb_drv_tcc3170_get_msc_ber(uint32 *msc_ber)
{
    ...
    TcbdReadSignalInfo(&TcbdDevice, &signalInfo);
...
int8 tunerbb_drv_tcc3170_tune(int nFreqNo, int skipLockWait)
{
    ...
    TcbdDebug(DEBUG_INFO, "tunerbb_drv_t3900_tune nFreqNo = %d, ulFreq = %d \n", (int)nFreqNo, (int)frequency);

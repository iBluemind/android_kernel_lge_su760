...
#define SIZE_MSC_STACK_BUFF    (1024*10)
#define SIZE_FIC_STACK_BUFF    (388*5)
#define SIZE_STATUS_STACK_BUFF (32*5)
...
inline static void TcbdStackChunk(ParserData_t *_parser, I08U *_buffer)
{
    ...
    TcbdDebug(0, "type:%d, buffer:0x%X currpos:%d, size:%d\n",
...
static I32S TcbdPushChunk(ParserData_t *_parser, I08U *_cachedBuffer)
{
    ...
    if(_parser->nextRead)
    {
        if(_parser->state != StateGarbage)
        {
            ...
            TcbdDebug(0, "send %s data %d bytes, pre:%d, curr:%d, buffer:0x%X\n",
    ...
    else
    {
        if(_parser->state != StateGarbage)
        {
            TcbdDebug(0, "keep %s data %d bytes buff:0x%X\n",

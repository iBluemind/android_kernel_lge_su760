...
I32S TcbdReadStream(TcbdDevice_t *_device, I08U *_buff, I32S *_size)
{
    ...
    bytesRemain = 0;
    TcbdDebug(0, "%d bytes read, %d bytes remain\n", bytesToRead, bytesRemain);
    *_size = bytesToRead;
...
I32S TcbdTuneFrequency(TcbdDevice_t *_device, I32U _freqKhz, I32S _bwKhz)
{
    ...
    {
        selectedBuffer = STREAM_DATA_ENABLE|STREAM_HEADER_ON|STREAM_MASK_BUFFERA;
        ret |= TcbdInitStreamDataConfig(_device, ENABLE_CMD_FIFO, selectedBuffer, TCBD_THRESHOLD_FIC);
    }
...
I32S TcbdRegisterService(TcbdDevice_t *_device, TcbdService_t *_service)
{
    ...
    I32U threshold = 0, garbage, dataMode = 0;
    ...
    switch(_device->periType)
    {
        case PeriTypeSpiOnly:
            ...
            threshold = TCBD_MAX_THRESHOLD;
            garbage = TCBD_MAX_THRESHOLD >> 2;
#endif/*__CALC_INTRRUPT_THRESHOLD__*/
            TcbdDebug(DEBUG_API_COMMON, "threshold : %d, garbage:%d\n", threshold, garbage);

            enCmdFifo = ENABLE_CMD_FIFO;
            selectedStream = ((garbage<<16) | STREAM_TYPE_GARBAGE) | (STREAM_TYPE_ALL & (~(STREAM_TYPE_STATUS)));
            selectedBuffer = STREAM_DATA_ENABLE|STREAM_HEADER_ON|STREAM_MASK_BUFFERA|STREAM_MASK_BUFFERB;
            break;
        case PeriTypeSts:
            enCmdFifo = DISABLE_CMD_FIFO;
            selectedStream = STREAM_TYPE_ALL & (~(STREAM_TYPE_STATUS));

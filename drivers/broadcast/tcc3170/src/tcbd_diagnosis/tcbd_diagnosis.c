...
static inline void TcbdCalcPcber(TcbdDiagnosis_t *_diagInfo)
{
    ...
    else if(_diagInfo->pcber > MAX_PCBER * SCALE_FACTOR)
        _diagInfo->pcber = MAX_PCBER*SCALE_FACTOR;
...
static inline void TcbdCalcViterbiber(TcbdDiagnosis_t *_diagInfo)
{
    ...
    {
        _diagInfo->vber = MAX_VITERBIBER * SCALE_FACTOR;
    ...
    rsOver = ((I32U)(currStatus->rsErrCnt - oldStatus->rsErrCnt))*SCALE_FACTOR;
    rsTemp = currStatus->rsPktCnt - oldStatus->rsPktCnt;
    rsUnder = rsTemp * 204 * 8;
    if(!rsUnder) return;
    _diagInfo->vber = rsOver / rsUnder;

    if(_diagInfo->vber < MIN_VITERBIBER)
        _diagInfo->vber = MIN_VITERBIBER;
    else if(_diagInfo->vber > MAX_VITERBIBER*SCALE_FACTOR)
        _diagInfo->vber = MAX_VITERBIBER*SCALE_FACTOR;
...
static inline void TcbdCalcTsper(TcbdDiagnosis_t *_diagInfo)
{
    ...
    if(Diagnosis.resynced)
    {
        _diagInfo->tsper = MAX_TSPER * SCALE_FACTOR;
        goto exitCalcTsper;
    }
    ...
    if(tsper < MIN_TSPER)
        _diagInfo->tsper = MIN_TSPER;
    else if(tsper > MAX_TSPER*SCALE_FACTOR)
        _diagInfo->tsper = MAX_TSPER*SCALE_FACTOR;

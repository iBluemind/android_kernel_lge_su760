...
static struct hciif_client *hciif_add_client_to_array(int channel, unsigned char evt_type[], struct hciif_client *existingClient);
#ifdef VERBOSE
 ...
 *           else suitable error code
 */
ssize_t hciif_read(struct file *file, char __user *data, size_t size, loff_t *offset)
{
	int len = 0;
	...
	if (skb->len > size) {
		HCIIFDRV_DBG("SKB length is Greater than requested size Returning the requested size");
 ...
 *           else suitable error code
 */
ssize_t hciif_write(struct file *file, const char __user *data,  size_t size, loff_t *offset)
{
	struct hciif_client 	*client;
	struct sk_buff 		*skb;
 ...
 *           else suitable error code
 */
static int hciif_ioctl(struct inode *inode, struct file *file,	unsigned int cmd, unsigned long arg)
{
	struct sk_buff *skb = NULL;
	...
	switch (cmd) {
		...
		case HCIIF_IOCTL_DEVUP:
        {
            ...
            copy_from_user(&filter, /*vic*/(const void __user *)arg, sizeof(struct hciif_filter_t));
...
static int __init hciif_init(void)
{
	 ...
	 */
	hhciif->hciif_major =  register_chrdev(0, DEVICE_NAME, &hciif_chrdev_ops);
	if (0 > hhciif->hciif_major) {
		err = hhciif->hciif_major;
		HCIIFDRV_ERR("Error when registering to char dev. Error = %ld.",  err);
		kfree(hhciif);
		return err;
	}
	...
	hhciif->hciif_dev = device_create(hhciif->hciif_class, NULL,
...
static void __exit hciif_exit(void)
{
	...
	HCIIFDRV_DBG(" Inside %s, freeing up: %d", __func__, hhciif->hciif_major);
...
int hciif_dev_up(struct file *file, struct hciif_filter_t *rcv_filter, struct hciif_client **client)
{
	...
	if(test_bit(HCIIF_CHAN_EVT, &filter.chan_mask))
	{
		...
		{
			HCIIFDRV_ERR("hciif_dev_up(): Trying to regsiter a client to Events channel --> Max clients are already registered");
		...
		if(retCode == HCIIF_SUCCESS)
		{
            HCIIFDRV_VER("hciif_dev_up(): calling hciif_add_client_to_array(HCIIF_CHAN_EVT) ");
	...
	for(i = 0; i < ST_MAX_CHANNELS; i++)   
	{
		...
		if(test_bit(i, &filter.chan_mask))
		{
			...
			else
			{
				...
				if(retCode == HCIIF_SUCCESS)
				{
                    HCIIFDRV_VER("hciif_dev_up()/hciif_register_client(OK): successfully registered client with index=%d\n", i);
...
long hciif_send_frame(struct sk_buff *skb, struct hciif_client *client)
{
    ...
    hciif_print_raw_data(skb->data, skb->len, "hciif_send_frame(To ST)");
#endif
...
long hciif_st_event_recv(void *priv_data, struct sk_buff *skb)
{
	...
	else
	{
		...
		if((skb->data[0] == HCIIF_OP_EVT_COMPLETED) || (skb->data[0] == HCIIF_OP_EVT_STATUS))
		{
			...
			if( (skb->data[0] == HCIIF_OP_EVT_COMPLETED) && ((skb->data[2] != 1) || (destClient->cmd_opcode != *((uint16_t *)(&skb->data[3])))) )
			{
				spin_unlock(&hhciif->hciif_lock);
				HCIIFDRV_ERR("*** ERROR HCIIF_OP_EVT_COMPLETED ***: The received opcode from client "
                             "0x%08x, is Cmd complete: 0x%04x, not matching the expected cmd opcode 0x%04x!!!!!", 
                             (unsigned int)destClient, *((uint16_t *)&skb->data[3]), destClient->cmd_opcode);
				return HCIIF_ERR_FAILURE;

			} 
            else if( (skb->data[0] == HCIIF_OP_EVT_STATUS) && ((skb->data[3] != 1) || (destClient->cmd_opcode != *((uint16_t *)(&skb->data[4])))) )
			{
				spin_unlock(&hhciif->hciif_lock);
				HCIIFDRV_ERR("***ERROR HCIIF_OP_EVT_STATUS ***: The received opcode from client "
                             "0x%08x, is Cmd complete: 0x%04x, not matching the expected cmd opcode 0x%04x!!!!!", 
                             (unsigned int)destClient, *((uint16_t *)&skb->data[4]), destClient->cmd_opcode);
				return HCIIF_ERR_FAILURE;
			}
...
long hciif_st_ganeric_recv(void *priv_data, struct sk_buff *skb)
{
	...
	if(!destClient)
	{
		HCIIFDRV_ERR("Can't send packet --> No client is registered to the requested channel %d", bt_cb(skb)->pkt_type);
		return HCIIF_ERR_FAILURE;
	}
...
static void hciif_print_raw_data(char *raw_data, int skb_len, char *func_name)
{
	...
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_NONE,	32, 1, raw_data, skb_len, 0);
...
static int hciif_register_client(struct st_proto_s *hciif_proto)
{
	...
	*/
	...
	if (ret == -EINPROGRESS) {
		...
		timeout = wait_for_completion_timeout(&hhciif->hciif_reg_completed,	msecs_to_jiffies(timeout));
		/* Check for timed out condition */
		if (0 == timeout) {
		...
		if (hhciif->streg_cbdata != 0) {
			HCIIFDRV_ERR("BT Device Registration Failed-ST Reg CB called with invalid value %d", hhciif->streg_cbdata);
			return -EAGAIN;
		}
	}
...
static int hciif_enter_cs(void)
{
	...
	while(test_and_set_bit(HCIIF_ST_INPROGRESS, &hhciif->state))
	{
		...
		timeout = wait_for_completion_timeout(&hhciif->hciif_oper_completed, msecs_to_jiffies(timeout));
...
//MODULE_DESCRIPTION("HCI IF for TI Shared Transport" VERSION);
